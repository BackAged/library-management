...
func (c *Client) UpdateOne(ctx context.Context, col string, f bson.M, u bson.M) (interface{}, error) {
	res, err := c.client.Database(c.db).Collection(col).UpdateOne(ctx, f, u)
	if err != nil {
		return nil, err
	}
